		...
		$.post($("#new_marker").attr("action"), $("#new_marker").serialize(), function(data) {
			...
			if (json.status == "success") {
				new Messi(lang['dist_marker_insertok'], {
					title: lang['dist_lbl_success'],
					titleClass: 'info',
					modal: true,
					buttons: [{
						id: 0,
						label: lang['dist_lbl_proceed'],
						val: 'X'
					}],
					callback: function() {
...
$(function() {
	$('#user_insert').submit(function(e) {
		...
		$.post($("#user_insert").attr("action"), $("#user_insert").serialize(), function(data) {
			var json = myParseJSON(data);
			if (json.status == "OK") {
				new Messi(lang['dist_newuser_ok2'], {
					title: lang['success'],
					titleClass: 'success',
					modal: true,
					buttons: [{
...
$(function() {
	$('#upload_marker_image').submit(function(e) {
		...
		if (max_images_marker != 0 && mrkImagesCount >= max_images_marker) {
			new Messi(lang['dist_imgupload_max'], {
				title: lang['error'],
				tttleClass: 'anim error',
				buttons: [{
					id: 0,
...
$(function() {
	$(document).on('click', '.delete_file_link', function(e) {
		e.preventDefault();
		var link = $(this);
		new Messi(lang['dist_imgdel_confirm'], {
			modal: true,
			buttons: [{
				id: 0,
				label: 'Sim',
...
$(function() {
	$(document).on('click', '.delete_marker_link', function(e) {
		e.preventDefault();
		var link = $(this);
		new Messi(lang['dist_mrkdel_confirm'], {
			modal: true,
			buttons: [{
				id: 0,
				label: 'Sim',
			...
			callback: function(val) {
				if (val == 'S') delete_marker_map(link);
			}
		});
		return false;
	});
...
function refresh_marker_images(marker_id) {
	$.get(site_root + 'image/list_marker_images/' + marker_id).success(function(data) {
